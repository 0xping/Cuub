...
/*   Updated: 2023/10/19 03:09:13 by aait-lfd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_surrounds(bool *valid, char **map, int i, int j)
{
	...
	*valid = (*valid && ft_strchr("NEWS1023", map[i + 1][j])
			&& ft_strchr("NEWS1023", map[i - 1][j]));
	*valid = (*valid && ft_strchr("NEWS1023", map[i][j + 1])
			&& ft_strchr("NEWS1023", map[i][j - 1]));
}
// *valid = (*valid && ft_strchr("NEWS10", map[i + 1][j + 1])
// 		&& ft_strchr("NEWS10", map[i - 1][j - 1]));
// *valid = (*valid && ft_strchr("NEWS10", map[i - 1][j - 1])
// 		&& ft_strchr("NEWS10", map[i + 1][j + 1]));
